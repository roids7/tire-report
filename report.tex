...
\begin{aligned}
	ext{크라운(Crown)}: &\quad \lambda_C, \varepsilon_C \; \Rightarrow\; B_C = f(\lambda_C, \varepsilon_C, \dot\gamma_C) \\
	ext{베이스(Base)}: &\quad \lambda_B, \varepsilon_B \; \Rightarrow\; B_B = f(\lambda_B, \varepsilon_B, \dot\gamma_B) \\
	ext{윙(Wing)}: &\quad \lambda_W, \varepsilon_W \; \Rightarrow\; B_W = f(\lambda_W, \varepsilon_W, \dot\gamma_W)
\end{aligned}
...
\begin{itemize}
  ...
  \item 비용 우선: $W$를 작게 설정 (\leq 50 \text{ mm}) - 대신 다이 내부 피드 블록 설계 최적화
\end{itemize}
