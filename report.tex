...
\maketitle
\begin{abstract}
본 문서는 타이어의 주요 공정(배합, 공압출, 가류)에 대한 조사 결과와 향후 보고서 작성 계획을 정리하였다.
\end{abstract}
...
\subsubsection{압출(Extrusion) — 트레드·사이드월·아펙스 등}

혼련된 고무를 압출기에 투입해 트레드, 사이드월, 아펙스(비드 필러) 등 형상 부품을 연속 성형한다. 트레드는 보통 캡/베이스(±윙)를 동시에 뽑는 공압출을 사용하며, 다이 통과 후 냉각·길이계측·절단을 거쳐 성형공정 투입용 길이로 준비한다. 
...
브라스 코팅 스틸 와이어를 규정 회수로 권선하여 비드 링을 만들고, 여기에 \textbf{아펙스(고경도 삼각 웨지)}와 채퍼/플리퍼/치퍼 등 보강 부품을 적층해 모듈화한다. 비드는 림과의 기계적 결합과 기밀 유지의 기초이므로 동심도·외경 공차를 엄격히 관리한다.

\textbf{아펙스의 역할}: 아펙스의 각도·높이·질량은 하부 사이드월의 강성 구배를 형성해 승차감·조종성·내구에 영향을 준다. 스티칭·열착 조건을 통해 박리나 미끼움을 방지한다.
...
\begin{itemize}
  ...
  \item \textbf{아펙스 미끼움·박리} → 온도·압력·스티치 재설정
\end{itemize}
...
가류공정은 타이어 제조의 마지막 단계로, 미가황 상태의 타이어에 열과 압력을 가하여 고무 분자 간에 \textbf{황화 결합(Sulfur Cross-linking)}을 형성시키는 과정이다. 이 과정을 통해 타이어는 탄력성과 내구성, 열저항성을 갖추게 된다.
...
\subsubsection{가류의 목적}

가류의 목적은 고무의 물리적 성질을 개선하여 실사용 조건에서도 형태와 성능을 유지하도록 하는 것이다. 가류되지 않은 고무는 점착성과 탄성이 떨어지며, 열에 약하다. 그러나 가류 후에는 강도, 탄성, 내마모성, 내열성이 크게 향상된다.

\subsubsection{가류설비}

타이어 가류에는 \textbf{벌카니제이션 프레스(Vulcanizing Press)}가 사용된다. 이 장비는 금형(mold) 내부에 고온과 고압을 가해 타이어를 일정 시간 동안 가열한다. 금형은 타이어의 외형과 트레드 패턴을 형성하며, 일반적으로 상·하 금형으로 구성된다.

\subsubsection{가류 조건}

가류 조건은 타이어의 종류와 크기에 따라 다르지만 일반적으로 다음과 같이 유지된다:
\begin{itemize}
  \item \textbf{온도}: 약 140\textasciitilde{}160°C
  \item \textbf{압력}: 약 15\textasciitilde{}20 bar
  \item \textbf{시간}: 약 10\textasciitilde{}15분
\end{itemize}

이 조건에서 고무 내부의 황(Sulfur)이 고무 분자와 반응하여 \textbf{황화 결합(S-S bond)}을 형성하게 된다.

\subsubsection{가류 과정의 단계}

\begin{enumerate}
  \item \textbf{타이어 장착}: 미가황 타이어를 금형 내부에 장착한다.
  
  \item \textbf{열 및 압력 가열}: 금형 내부에서 타이어가 고온·고압 상태로 가열되며, 내부의 블래더(bladder)가 팽창해 타이어를 금형 벽면에 밀착시킨다.
  
  \item \textbf{황화 반응 진행}: 고무 분자 간에 황 결합이 형성되면서 고무가 탄력 있고 강한 3차원 구조로 변화한다.
  
  \item \textbf{냉각 및 탈형}: 가류가 완료되면 금형을 열고, 타이어를 서서히 냉각시킨 뒤 금형에서 꺼낸다. 급격한 냉각은 내부 응력을 유발할 수 있으므로 천천히 냉각하는 것이 중요하다.
\end{enumerate}
...
\subsection{배합 재료의 선정}

\subsection{배합비 최적화}

\subsection{혼련 공정}

\subsection{배합물의 물성 평가}
...
\subsection{가류 특성 평가}
\subsection{가류가 타이어 물성에 미치는 영향}

\section{결론}

본 보고서는 타이어를 다층 복합재로 보고, 정련·압출·압연·반제품·성형·가류·검사에 이르는 전체 공정을 계통적으로 정리하였다. 특히 공정 능력 관점에서 트레드 공압출과 가류 공정을 중점적으로 분석하였다.

\subsection{주요 성과}

\subsubsection{공정 체인 이해}

흐름을 단계별로 배열함으로써, 개별 공정이 아닌 공정 체인으로 이해할 수 있게 되었다. 배합 설계에서 시작하여 공압출과 가류를 거쳐 최종 검사에 이르는 전 과정이 상호 연결되어 있음을 확인하였다.

\subsubsection{균일성과 품질 지표의 연결}

공정 파라미터가 균일성 향상이라는 품질 지표를 유동 현상과 직접 연결할 수 있었다. 특히 다이 팽윤(식 \ref{eq:swell}), 층간 팽윤 차이, 벽면 전단률 등이 최종 제품의 균일성에 미치는 영향을 정량화할 수 있었다.

\subsubsection{공정 설계 원리의 적용}

기계가공과 공정설계 과목에서 다루는 공정 능력·변동 관리 개념을 실제 산업 공정에 접목하였다. 연속체 역학의 기본 원리(연속 방정식, 운동량 방정식 \ref{eq:momentum_steady})와 점탄성 구성식(PTT 모델 \ref{eq:ptt})을 통해 고무 용융체의 비뉴턴 거동을 체계적으로 분석할 수 있었다.

\subsubsection{수치해석과 실무의 연계}

식 (\ref{eq:die_width_effects}), (\ref{eq:convergence_angle_effects}), (\ref{eq:flow_rate_effects}), (\ref{eq:draw_speed_swell})을 통해 다이 너비, 수렴각, 유량, 인발 속도 등 주요 설계 변수가 공정 결과에 미치는 경향성을 파악할 수 있었다.

\subsection{한계와 개선 방안}

\subsubsection{모델의 한계}

구성식 모델의 한계로 인하여 정량적인 예측보다는 경향성 파악에 가까운 수준에 머물렀다. 이는 시뮬레이션 결과를 바로 공정 조건의 절대 기준으로 사용하기 어렵다는 한계를 의미한다. 따라서 현장 검증 및 보정이 필수적이다.

\subsubsection{공정 범위의 한계}

타이어의 균일성을 논의하면서, 주로 트레드 공압출과 가류 공정에 초점을 맞춘 것은 선택과 집중 차원에서는 효과적이지만, 배합 설계, 캘린더링, 성형 등 모든 공정이 균일성에 미치는 영향을 고려하지 않았다는 한계가 있다.

\subsubsection{향후 개선 방향}

이러한 성과와 한계를 바탕으로, 향후에는 다음의 개선이 필요함을 확인하였다:

\begin{enumerate}
  \item \textbf{실험 데이터 기반 보정}: 실제 공정에서 수집한 압력, 온도, 형상 데이터를 이용하여 시뮬레이션 모델의 파라미터 최적화
  
  \item \textbf{고급 구성식의 적용}: PTT 모델을 넘어 multi-mode 점탄성 모델이나 비선형 비탄성 모델 도입으로 더 높은 정확도 추구
  
  \item \textbf{통합 공정 분석}: 배합부터 가류까지 전 공정을 연결한 통합 시뮬레이션을 통해 각 공정의 상호작용 분석
  
  \item \textbf{경제·안전성 평가}: 균일성 향상에 따른 품질 개선과 생산 비용, 설비 안전성을 균형있게 평가하는 의사결정 체계 구축
\end{enumerate}

\subsection{최종 결론}

본 보고서는 타이어 공정의 복잡한 유동 현상을 연속체 역학과 점탄성 이론으로 체계화하고, 공정 파라미터 최적화를 위한 실무적 지침을 제시하였다. 특히 균일성이라는 품질 지표를 중심으로 배합부터 가류까지의 공정 체인을 분석함으로써, 시행착오 없이 고품질 타이어를 효율적으로 생산할 수 있는 기초를 마련하였다.

향후 현장 데이터와 더 고급 모델의 결합, 그리고 실제 생산 라인을 고려한 경제·안전성 평가가 병행될 때, 본 보고서의 이론적 기초가 실제 공정 혁신으로 이어질 수 있을 것으로 기대된다.
...
\begin{enumerate}
  ...
  \item Mark, J.E., \textit{The Science and Technology of Rubber}, Academic Press.
  \item Bhowmick \& Stephens, \textit{Handbook of Elastomers}, CRC Press.
  \item \textit{Rubber Extrusion Handbook}, Hanser Publishers.
  \item Montgomery, \textit{Introduction to Statistical Quality Control}, Wiley.
