...
타이어는 단순 고무가 아닌 다층 복합재로 구성되며, 각 층이 서로 다른 기능을 담당한다:

\begin{itemize}
  \item \textbf{트레드(Tread)}: 노면 접촉으로 마모 저항성, 그립감 담당
  \item \textbf{보강층(Reinforcement)}: 카카스와 벨트로 구조적 강도 제공
  \item \textbf{사이드월(Sidewall)}: 주행 중 부하 변화에 대응
  \item \textbf{비드(Bead)}: 림과의 접속부, 공기 누설 방지
\end{itemize}

\subsection{트레드의 다층 구조}

트레드는 마모층(Crown), 기초층(Base), 윙(Wing) 등 3중 이상의 고무층으로 구성되며, 각 층의 고무 배합이 상이하다.
