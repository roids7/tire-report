...
\textbf{장비}: 콜드피드 단축(다축) 압출기 + 기어펌프 + 스크린팩 + 다층 헤드(듀플렉스/트리플렉스) + 다이 + 인라인 레이저/비전 + 컷투렝스(Cut-to-Length).

\textbf{핵심}: 프로파일/게이지 정합, 층간 접착(공압출 계면), 스코치 안전, 절단 길이·센터링 동기.
...
\textbf{장비}: 2--4롤 캘린더 + 피드밀 + 코드 가이드·텐셔너 + 게이지 스캐너(상·하/총두께) + 와인더.

\textbf{핵심}: 게이지 균일성, 코드 각도/EPI 분포, 접착(브라스–고무·RFL), 열형상(정지·재기동 관리).
...
\subsubsection{비드 공정(Bead) — 링/필러 모듈}

\textbf{장비}: 비드 와인더(권선)·성형/프레스 + 아펙스 압출 + 차퍼 스트립 적층 + 동심·외경 검사.

\textbf{핵심}: 동심도/외경 공차, 아펙스 각도·높이·질량(턴업부 강성·승차감), 접착 강도.
...
\subsection{성형 공정(Building)}

준비된 반제품(이너라이너, 플라이, 벨트, 트레드, 사이드월, 비드 등)을 드럼 또는 성형기에서 순차적으로 적층하여 그린 타이어(Green Tire)를 완성하는 공정이다. 성형 과정에서 각 부품의 위치 정렬과 접착이 중요하다.

\subsection{가류 공정(Curing/Vulcanization)}

그린 타이어를 몰드에 넣고 고온·고압(약 150--180°C, 15--20 bar)에서 가황 반응을 일으켜 고무에 탄성과 강도를 부여하는 공정이다. 이 과정에서 트레드 패턴이 몰드에 의해 형성된다.

\subsection{검사 공정(Inspection)}

가류가 완료된 타이어는 외관 검사, 유니포미티(Uniformity) 검사, X-ray 검사 등을 거쳐 품질을 확인한다. 불량 타이어는 선별되고, 합격한 제품만 출하된다.
...
\subsubsection{연속 방정식 (Continuity Equation)}
...
\subsection{PTT 점탄성 구성식과 다이 팽윤}

\subsubsection{편차응력의 분류}
...
\subsection{유동 특성과 다이 설계}

\subsubsection{유동 균일성 지수}
...
\subsection{다이 구조 설계 및 최적화}

\subsubsection{다이 구조 파라미터}
...
\subsection{다이 설계 최적화 및 계면 동역학}

\subsubsection{다중 목적 함수 최적화}
...
\subsection{파레토 최적해(Pareto Optimal Solution)}
...
\begin{enumerate}
  ...
  \item \textbf{공정 최적화 전략}: 다중 목적 최적화 제약 조건 하에서 파레토 최적해를 도출하는 방법론을 제시했다.
\end{enumerate}
