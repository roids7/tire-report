...
% 한글 설정: fontspec + 윈도우 기본 폰트 (Malgun Gothic)
\usepackage{fontspec}
\usepackage{xeCJK}
...
수렴각 증가 → 벽면 전단률 급증 → 표면 품질 저하. 동시에 압출 방향에 따라 유동 분배가 급격하게 변하는 "고전단 영역"이 입구에서 출구로 이동하면서 형상 편차도 증가한다.
...
\begin{equation}
\text{크라운(Crown)}: & \lambda_C, \varepsilon_C \quad \Rightarrow B_C = f(\lambda_C, \varepsilon_C, \dot\gamma_C) \\
\text{베이스(Base)}: & \lambda_B, \varepsilon_B \quad \Rightarrow B_B = f(\lambda_B, \varepsilon_B, \dot\gamma_B) \\
\text{윙(Wing)}: & \lambda_W, \varepsilon_W \quad \Rightarrow B_W = f(\lambda_W, \varepsilon_W, \dot\gamma_W)
\end{equation}
...
\begin{enumerate}
  ...
  \item \textbf{공정 최적화 전략}: 식 (\ref{eq:multi_objective})와 (\ref{eq:constraints})의 제약 조건 하에서, 파렛토 최적해를 도출하는 방법론을 제시했다.
\end{enumerate}
...
권장사항:
\begin{itemize}
  \item 형상 정밀도 우선: $W$를 크게 설정 (≥100 mm)
  \item 표면 품질 중시: $W$를 중간 범위 유지 (50-80 mm)
  \item 비용 우선: $W$를 작게 설정 (≤50 mm) - 대신 다이 내부 피드 블록 설계 최적화
\end{itemize}
...
본 보고서는 연속체 역학의 기본 원리(지배방정식 \ref{eq:continuity_material}-\ref{eq:stress_decomposition}), 점탄성 구성식(PTT 모델 \ref{eq:ptt}), 수치해석 기법(유한 요소법, 뉴턴-랍슨 반복)을 종합적으로 적용하여, 타이어 트레드 3중 공압출 공정의 설계와 최적화에 대한 이론적 기초와 실무적 지침을 제시했다.
